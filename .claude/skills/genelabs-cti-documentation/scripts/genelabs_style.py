...
def build_footer(doc, doc_id, doc_marking, tlp=DEFAULT_TLP,
    ...
    for section in doc.sections:
        ...
        # Internal program documentation carries NO TLP marking. TLP is a
        # sharing designation, not a classification: it belongs on bulletins
        # and on anything leaving GeneLabs, not on the plans. Pass
        # tlp=None or tlp="none" to omit it.
        show_tlp = bool(tlp) and str(tlp).strip().lower() not in ("none", "off", "")
        if show_tlp:
