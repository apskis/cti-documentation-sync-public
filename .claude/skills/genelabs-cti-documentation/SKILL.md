...
| `--tlp` | Footer TLP level. Default `none`. Internal program documentation carries NO TLP marking: TLP is a sharing designation, not a classification, so it belongs on bulletins and on anything leaving GeneLabs, not on the plans. Pass `AMBER` or `AMBER+STRICT` only for a document going outside. |
